...
export function getCompressPngImagesOnlineContent(): RouteEditorialContent {
  return {
    ...
    steps: [
      ...
      'Export optimized PNG assets directly to your local device without network data transfer.'
    ],
    faqs: [
      ...
      makeFaq('Are my PNG files uploaded to remote servers during compression?', 'No. All PNG optimization scripts execute locally within your browser WebAssembly sandbox. Zero image bytes leave your device.')
    ]
  };
}
...
export function getPageSeo(fullUrl: string): SeoRouteData {
  ...
  return {
    ...
    metaDescription: 'Compress PNG images losslessly or using palette quantization in browser memory. Preserves full alpha transparency with zero server uploads.',
    ...
    jsonLd: generateJsonLdSchemas(
      ...
      'Compress PNG images losslessly in browser RAM with full alpha transparency and zero server data transfers.',
      fullUrl,
      guideContent.faqs,
