...
export function getConvertTiffBmpToJpgContent(): RouteEditorialContent {
  return {
    ...
    section1Body: 'Legacy hardware scanners, medical imaging devices, and older Windows software output uncompressed bitmap (BMP) or tagged image file format (TIFF) files. These formats carry high byte overhead and are rejected by web forms and social platforms. Zapixal decodes raw uncompressed pixel arrays in local browser memory and re-encodes them into baseline JPEG using MozJPEG WebAssembly binaries, dramatically reducing byte size.',
    section2Title: 'Processing legacy scan archives without remote data transmission',
    section2Body: 'Medical records, legal documents, and historical scans saved as multi-megabyte TIFF or BMP files create massive storage bottlenecks. Converting them to optimized JPEG format shrinks files by 80% to 95% while retaining legible text and fine details. Because the conversion pipeline runs entirely within your browser tab, sensitive documents remain securely sandboxed on your local hardware.',
    steps: [
      'Drag TIFF or BMP files directly into the browser converter window.',
...
export function getPageSeo(fullUrl: string): SeoRouteData {
  ...
  return {
    ...
    jsonLd: generateJsonLdSchemas(
      ...
      'Convert uncompressed TIFF and BMP files to lightweight JPEG format in local browser RAM with zero remote data transmission.',
      fullUrl,
      guideContent.faqs,
