...
export function DonateModal({ isOpen, onClose }: DonateModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-6 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200 overflow-y-auto">
      <div className="relative w-full max-w-lg mx-2 p-5 sm:p-8 bg-white dark:bg-[#303134] border border-neutral-200 dark:border-[#3c4043] rounded-2xl sm:rounded-3xl shadow-2xl animate-in zoom-in-95 duration-200 my-auto">
        <button 
          onClick={onClose}
          className="absolute top-3 right-3 p-1.5 text-neutral-400 hover:text-neutral-700 dark:text-[#9aa0a6] dark:hover:text-[#e8eaed] hover:bg-neutral-100 dark:hover:bg-[#3c4043] rounded-full transition-colors cursor-pointer"
        >
              ...
              <div className="flex items-center gap-2.5 sm:gap-4 min-w-0">
                ...
                <div className="flex flex-col items-start min-w-0">
                  <div className="flex items-baseline gap-1.5 flex-wrap">
                    <span className="font-extrabold text-neutral-900 dark:text-[#e8eaed] text-base sm:text-lg leading-tight">
                      {plan.amount}
                    </span>
                    <span className="text-[10px] sm:text-xs font-semibold text-neutral-400 dark:text-[#9aa0a6]">
                      ({plan.type})
                    </span>
                  </div>
                  <span className="text-neutral-500 dark:text-[#9aa0a6] text-xs sm:text-sm font-semibold truncate w-full mt-0.5">
                    {plan.description}
                  </span>
                </div>
              </div>
