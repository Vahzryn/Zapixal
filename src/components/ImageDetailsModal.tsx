...
export function ImageDetailsModal({ item, onClose }: ImageDetailsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 bg-black/85 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative flex flex-col w-full max-w-6xl h-[95vh] max-h-[920px] bg-white dark:bg-[#202124] rounded-2xl sm:rounded-3xl shadow-2xl border border-neutral-200 dark:border-[#3c4043] overflow-hidden">
        
        {/* Header with Title + Interactive Zoom Controls */}
        <div className="flex flex-wrap items-center justify-between gap-2 px-3 sm:px-6 py-2 sm:py-3 border-b border-neutral-200 dark:border-[#3c4043] bg-neutral-50/90 dark:bg-[#28292c]/90 backdrop-blur-md z-10 shrink-0">
          
          {/* File Title Info */}
          <div className="flex items-center gap-2 sm:gap-3 min-w-0 max-w-xs sm:max-w-md">
            <div className="p-1.5 sm:p-2 bg-indigo-50 dark:bg-[#21243a] text-indigo-600 dark:text-[#a8b1ff] rounded-lg sm:rounded-xl shrink-0">
              <Info className="w-4.5 h-4.5" />
            </div>
            <div className="min-w-0">
              <h3 className="text-sm sm:text-lg font-bold text-neutral-900 dark:text-[#e8eaed] truncate">
                {item.file.name}
              </h3>
              <p className="text-[10px] sm:text-xs text-neutral-500 dark:text-[#9aa0a6] truncate">
          ...
          <div className="flex items-center gap-1 sm:gap-1.5 bg-neutral-200/60 dark:bg-[#3c4043]/60 p-0.5 sm:p-1 rounded-xl sm:rounded-2xl border border-neutral-300/50 dark:border-[#5f6368]/50">
            
            <button
              onClick={handleZoomOut}
              className="p-1 sm:p-1.5 text-neutral-700 dark:text-[#e8eaed] hover:bg-white dark:hover:bg-[#202124] rounded-lg sm:rounded-xl transition-all active:scale-95 cursor-pointer"
              title="Zoom Out (-)"
              aria-label="Zoom Out"
            >
            ...
            <span className="px-1.5 text-[10px] sm:text-xs font-mono font-bold text-neutral-800 dark:text-[#e8eaed] min-w-[2.25rem] sm:min-w-[3.25rem] text-center">
              {zoomPercent}%
            </span>
            ...
            <div className="w-px h-3 bg-neutral-300 dark:bg-[#5f6368] mx-0.5" />
 
            <button
              onClick={handleFitToScreen}
              className="px-1.5 py-0.5 sm:px-2.5 sm:py-1 text-[10px] sm:text-xs font-bold text-neutral-700 dark:text-[#e8eaed] hover:bg-white dark:hover:bg-[#202124] rounded-lg sm:rounded-xl transition-all cursor-pointer flex items-center gap-0.5"
          ...
          <div className="flex items-center gap-1 sm:gap-1.5">
            ...
            <button
              onClick={onClose}
              className="p-1.5 transition-all text-neutral-500 hover:text-neutral-900 dark:text-[#9aa0a6] dark:hover:text-[#e8eaed] hover:bg-neutral-200 dark:hover:bg-[#3c4043] rounded-full cursor-pointer"
        ...
        <div className="relative flex-1 flex flex-col lg:flex-row overflow-hidden bg-neutral-950">
          ...
          {showInfoPanel && (
            <div className="w-full lg:w-80 border-t lg:border-t-0 lg:border-l border-neutral-200 dark:border-[#3c4043] bg-white dark:bg-[#202124] p-4 sm:p-5 space-y-4 sm:space-y-5 overflow-y-auto shrink-0 max-h-[35vh] lg:max-h-none">
              
              <div className="flex items-center justify-between">
                <h4 className="text-xs sm:text-sm font-bold text-neutral-900 dark:text-[#e8eaed] flex items-center gap-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="p-2.5 bg-neutral-50 dark:bg-[#303134] border border-neutral-200 dark:border-[#3c4043] rounded-xl sm:rounded-2xl">
                  <span className="block text-[9px] font-bold text-neutral-400 dark:text-[#9aa0a6] uppercase tracking-wider">File Size</span>
                  <span className="text-[11px] sm:text-xs font-black text-neutral-800 dark:text-[#e8eaed] mt-0.5 block">
                    {formatBytes(item.originalSize)}
                  </span>
                </div>
 
                <div className="p-2.5 bg-neutral-50 dark:bg-[#303134] border border-neutral-200 dark:border-[#3c4043] rounded-xl sm:rounded-2xl">
                  <span className="block text-[9px] font-bold text-neutral-400 dark:text-[#9aa0a6] uppercase tracking-wider">Dimensions</span>
                  <span className="text-[11px] sm:text-xs font-black text-neutral-800 dark:text-[#e8eaed] mt-0.5 block">
                ...
                <div className="p-2.5 bg-neutral-50 dark:bg-[#303134] border border-neutral-200 dark:border-[#3c4043] rounded-xl sm:rounded-2xl">
                  <span className="block text-[9px] font-bold text-neutral-400 dark:text-[#9aa0a6] uppercase tracking-wider">Aspect Ratio</span>
                  <span className="text-[11px] sm:text-xs font-black text-blue-600 dark:text-[#8ab4f8] mt-0.5 block">
                    {imgDimensions ? calculateAspectRatio(imgDimensions.width, imgDimensions.height) : '-'}
                  </span>
                </div>
 
                <div className="p-2.5 bg-neutral-50 dark:bg-[#303134] border border-neutral-200 dark:border-[#3c4043] rounded-xl sm:rounded-2xl">
                  <span className="block text-[9px] font-bold text-neutral-400 dark:text-[#9aa0a6] uppercase tracking-wider">MIME Type</span>
                  <span className="text-[11px] sm:text-xs font-black text-neutral-800 dark:text-[#e8eaed] mt-0.5 block uppercase truncate">
              ...
              <div className="p-3 sm:p-4 bg-neutral-50 dark:bg-[#303134] border border-neutral-200 dark:border-[#3c4043] rounded-xl sm:rounded-2xl">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-1">
                    <Palette className="w-3.5 h-3.5 text-indigo-600 dark:text-[#a8b1ff]" />
                    <h4 className="text-[11px] sm:text-xs font-bold text-neutral-800 dark:text-[#e8eaed]">Extracted Palette</h4>
                  </div>
                  <span className="text-[9px] font-medium text-neutral-400">Click color</span>
                </div>
                ...
                {colors.length > 0 ? (
                  <div className="grid grid-cols-3 gap-1.5">
                    {colors.map((hex) => (
                      <button
                        ...
                        className="flex flex-col items-center p-1 rounded-lg bg-white dark:bg-[#202124] border border-neutral-200 dark:border-[#3c4043] hover:border-indigo-500 transition-all group cursor-pointer"
                      >
                        <div
                          className="w-full h-6 rounded shadow-inner mb-0.5 border border-black/10"
                          style={{ backgroundColor: hex }}
                        />
                        <span className="text-[9px] font-mono font-bold text-neutral-700 dark:text-[#e8eaed] group-hover:text-indigo-600 flex items-center gap-0.5">
                          {copiedColor === hex ? (
                            <>
                  ...
                  <p className="text-[10px] text-neutral-400 italic">Analyzing palette...</p>
                )}
              </div>
