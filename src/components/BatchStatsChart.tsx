...
export function BatchStatsChart({ files }: BatchStatsChartProps) {
  ...
  const chartData = convertedFiles.map((file) => {
  ...
  const pieData = totalSaved >= 0 
    ? [
        { name: 'Converted Size', value: totalConverted, color: '#3b82f6' },
        { name: 'Space Saved', value: totalSaved, color: '#10b981' },
      ]
    : [
        { name: 'Original Size', value: totalOriginal, color: '#64748b' },
        { name: 'Size Added', value: Math.abs(totalSaved), color: '#f59e0b' },
      ];

  return (
    <div className="w-full mt-10 p-6 md:p-8 bg-white dark:bg-[#303134] border border-neutral-200 dark:border-[#3c4043] rounded-3xl shadow-sm transition-all min-h-[400px]">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-6 border-b border-neutral-100 dark:border-[#3c4043]">
        ...
        <div className={cn(
          ...
          totalSaved >= 0 
            ? "bg-emerald-500/10 dark:bg-[#1a2c20] border-emerald-500/20" 
            : "bg-amber-500/10 dark:bg-[#2c221a] border-amber-500/20"
        )}>
          <Zap className={cn("w-5 h-5", totalSaved >= 0 ? "text-emerald-600 dark:text-[#81c995]" : "text-amber-600 dark:text-[#fdd663]")} />
          <div>
            <span className="block text-[11px] font-bold text-neutral-500 dark:text-[#9aa0a6] uppercase tracking-wider">
              {totalSaved >= 0 ? "Total Space Saved" : "Total Size Increased"}
            </span>
            {totalSaved >= 0 ? (
              ...
              <span className="text-lg font-black text-amber-600 dark:text-[#fdd663]">
                +{formatBytes(Math.abs(totalSaved))} (+{Math.abs(overallSavingsPercent)}% larger)
              </span>
            )}
          </div>
        </div>
        ...
        <div className="p-4 bg-neutral-50 dark:bg-[#202124] border border-neutral-200 dark:border-[#3c4043] rounded-2xl">
          ...
          <span className={cn(
            ...
            overallSavingsPercent >= 0 
              ? "text-emerald-600 dark:text-[#81c995]" 
              : "text-amber-600 dark:text-[#fdd663]"
          )}>
            {overallSavingsPercent >= 0 ? `${overallSavingsPercent}%` : `+${Math.abs(overallSavingsPercent)}% increase`}
          </span>
        </div>
      </div>
