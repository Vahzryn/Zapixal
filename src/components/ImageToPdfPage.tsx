...
export const ImageToPdfPage: React.FC<ImageToPdfPageProps> = ({ seoData, onNavigate }) => {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-2 sm:px-4 py-2 sm:py-4 space-y-8">
      ...
      <div className="bg-white dark:bg-[#1e2024] rounded-3xl border border-neutral-200/80 dark:border-neutral-800 shadow-xl overflow-hidden p-6 sm:p-8 space-y-8">
        {!successResult && (
          <div 
          ...
          >
            <div className="w-12 h-12 rounded-xl bg-blue-100 dark:bg-blue-900/40 text-blue-600 dark:text-blue-400 flex items-center justify-center mb-2">
        ...
        {items.length > 0 && !successResult && (
          <div className="space-y-6">
            <div className="p-4 rounded-xl bg-neutral-50 dark:bg-neutral-900/50 border border-neutral-200 dark:border-neutral-800 flex flex-col gap-4">
              <div className="flex flex-col sm:flex-row gap-4">
                <div className="flex-1 space-y-2">
                  <label className="text-xs font-bold text-neutral-700 dark:text-neutral-300 uppercase tracking-wider">Output Mode</label>
                  <select 
                    value={mergeMode}
                    onChange={(e) => setMergeMode(e.target.value as any)}
                    className="w-full bg-white dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700 rounded-lg px-3 py-2 text-sm text-neutral-900 dark:text-white"
                  >
                ...
                <div className="flex-1 space-y-2">
                  <label className="text-xs font-bold text-neutral-700 dark:text-neutral-300 uppercase tracking-wider">Page Size</label>
                  <select 
                    value={pageSize}
            ...
            <div className="space-y-2">
              {items.map((item, index) => (
                <div key={item.id} className="flex items-center gap-3 p-3 rounded-xl bg-white dark:bg-[#25282c] border border-neutral-200 dark:border-neutral-700/80 group transition-all hover:shadow-sm">
                  <div className="w-12 h-12 rounded-lg overflow-hidden bg-neutral-100 dark:bg-neutral-800 shrink-0 border border-neutral-200 dark:border-neutral-700">
                  ...
                  {mergeMode === 'single' && (
                    <div className="flex items-center gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                      <button
                        ...
                        className="p-1.5 text-neutral-500 hover:text-neutral-700 dark:hover:text-neutral-300 disabled:opacity-30 disabled:hover:text-neutral-500 rounded bg-neutral-100 dark:bg-neutral-800"
                      >
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4 border-t border-neutral-200 dark:border-neutral-800">
              ...
              <button
                ...
                className="w-full sm:w-auto px-6 py-3 rounded-xl bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white font-bold text-sm shadow-lg shadow-blue-600/25 transition-all flex items-center justify-center gap-2"
              >
                {isProcessing ? (
                  <>
        ...
        {successResult && (
          <div className="flex flex-col items-center gap-6 py-4 max-w-lg mx-auto text-center animate-in fade-in zoom-in-98 duration-300">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-neutral-500 dark:text-neutral-400">
                Combined <span className="font-semibold text-neutral-800 dark:text-neutral-200">{items.length} {items.length === 1 ? 'image' : 'images'}</span> into <span className="font-mono text-neutral-700 dark:text-neutral-300">{successResult.filename}</span> ({formatBytes(successResult.size)}).
              </p>
            </div>
