...
export function InstallModal({ isOpen, onClose, deferredPrompt, onInstallSuccess }: InstallModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-black/60 backdrop-blur-xs animate-in fade-in duration-200">
      <div 
        className="relative w-full max-w-lg mx-2 bg-white dark:bg-[#202124] border border-neutral-200 dark:border-[#3c4043] rounded-2xl shadow-2xl overflow-hidden text-neutral-900 dark:text-[#e8eaed]"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header Banner */}
        <div className="relative p-5 sm:p-6 bg-gradient-to-br from-blue-600 via-indigo-600 to-blue-700 dark:from-[#1a233a] dark:to-[#1e293b] text-white">
          <button
            onClick={onClose}
          ...
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl sm:rounded-2xl bg-white/10 dark:bg-white/5 border border-white/20 flex items-center justify-center shrink-0 shadow-inner">
              <img 
                src={logoImg} 
                alt="Zapixal" 
                width="40"
                ...
                className="w-8 h-8 sm:w-10 sm:h-10 object-contain rounded-lg sm:rounded-xl" 
              />
            </div>
            <div>
              <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-white/20 text-white text-[9px] sm:text-[10px] font-bold uppercase tracking-wider mb-1">
                <DownloadCloud className="w-3 h-3" /> Progressive Web App
              </div>
              <h3 className="text-lg sm:text-xl font-extrabold tracking-tight">Install Zapixal App</h3>
              <p className="text-[11px] sm:text-xs text-blue-100 dark:text-neutral-300 mt-0.5 leading-tight">
        ...
        <div className="p-5 sm:p-6 space-y-4 sm:space-y-5">
          <div className="grid grid-cols-3 gap-2 sm:gap-3">
            <div className="p-2 sm:p-3 rounded-xl bg-neutral-50 dark:bg-[#303134] border border-neutral-100 dark:border-[#3c4043] text-center min-w-0">
              <Zap className="w-4 h-4 sm:w-5 sm:h-5 text-amber-500 mx-auto mb-1" />
              <div className="text-[10px] sm:text-xs font-bold truncate">Instant Launch</div>
              <div className="text-[8px] sm:text-[10px] text-neutral-500 dark:text-[#9aa0a6] truncate">From Dock / Desktop</div>
            </div>
            <div className="p-2 sm:p-3 rounded-xl bg-neutral-50 dark:bg-[#303134] border border-neutral-100 dark:border-[#3c4043] text-center min-w-0">
              <ShieldCheck className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-500 mx-auto mb-1" />
              <div className="text-[10px] sm:text-xs font-bold truncate">Works Offline</div>
              <div className="text-[8px] sm:text-[10px] text-neutral-500 dark:text-[#9aa0a6] truncate">No internet</div>
            </div>
            <div className="p-2 sm:p-3 rounded-xl bg-neutral-50 dark:bg-[#303134] border border-neutral-100 dark:border-[#3c4043] text-center min-w-0">
              {deviceType === 'desktop' ? (
                <Monitor className="w-4 h-4 sm:w-5 sm:h-5 text-blue-500 mx-auto mb-1" />
              ) : (
                <Smartphone className="w-4 h-4 sm:w-5 sm:h-5 text-blue-500 mx-auto mb-1" />
              )}
              <div className="text-[10px] sm:text-xs font-bold truncate">Zero Uploads</div>
              <div className="text-[8px] sm:text-[10px] text-neutral-500 dark:text-[#9aa0a6] truncate">100% Private</div>
            </div>
          </div>
