...
function FileItemComponent({ 
  ...
  return (
    <div 
      ...
      className={cn(
        "flex flex-col p-2.5 sm:p-3 transition-all bg-white dark:bg-[#303134] border-2 rounded-xl sm:rounded-2xl group shadow-2xs select-none gap-1.5 sm:gap-2 min-w-0 w-full",
    ...
    >
      ...
      <div className="flex items-center justify-between min-w-0 w-full gap-2">
        <div className="flex items-center flex-1 min-w-0 gap-2">
          ...
          {onToggleSelect && (
            <div className="shrink-0 flex items-center justify-center pl-0.5">
              ...
              >
                {isSelected && <Check className="w-2.5 h-2.5 text-white dark:text-neutral-900" strokeWidth={3} />}
              </div>
            </div>
          )}
          ...
          <div className="relative shrink-0 w-10 h-10 sm:w-12 sm:h-12 overflow-hidden rounded-lg sm:rounded-xl bg-neutral-100 dark:bg-[#202124] border border-neutral-200/60 dark:border-[#3c4043] shadow-xs flex items-center justify-center font-sans">
            {previewSrc && !hasImgError ? (
              ...
              <div className="flex flex-col items-center justify-center w-full h-full bg-blue-50 dark:bg-[#1e293b] text-blue-600 dark:text-[#8ab4f8]">
                ...
                <span className="text-[7px] sm:text-[8px] font-bold uppercase tracking-wider mt-0.5 max-w-[32px] sm:max-w-[40px] truncate">
          ...
          <div className="flex flex-col min-w-0 flex-1 justify-center font-sans">
            <span className="text-xs sm:text-sm font-bold truncate text-neutral-800 dark:text-[#e8eaed] mb-0.5" title={item.file.name}>
              {item.file.name}
            </span>

            <div className="flex flex-wrap items-center gap-1 sm:gap-1.5 text-[10px] sm:text-xs font-semibold text-neutral-500 dark:text-[#9aa0a6]">
              <span className="shrink-0">{formatBytes(item.originalSize)}</span>
              {rotation !== 0 && (
                <span className="text-[10px] sm:text-[11px] font-semibold text-indigo-600 dark:text-[#a8b1ff] shrink-0">
                  • {rotation}°
                </span>
              )}
              {isComplete && item.convertedSize && (
                <>
                  <ArrowRight className="w-2.5 h-2.5 sm:w-3 sm:h-3 shrink-0 text-neutral-400 dark:text-[#9aa0a6]" />
        ...
        <div className="shrink-0 flex items-center pl-1">
          {isProcessing && (
            <Loader2 className="w-4 h-4 sm:w-5 sm:h-5 animate-spin text-blue-500 dark:text-[#8ab4f8] shrink-0" />
          )}
          {isComplete && (
            <CheckCircle2 className="w-4 h-4 sm:w-5 sm:h-5 text-emerald-500 dark:text-[#81c995] shrink-0" />
          )}
          {isError && (
      ...
      <div className="flex flex-col gap-1.5 sm:gap-2 pt-1.5 sm:pt-2 border-t border-neutral-100 dark:border-[#3c4043]/60 min-w-0 w-full mt-0">
        ...
        <div className="flex flex-wrap items-center justify-between gap-1.5 w-full min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-1 sm:gap-1.5 min-w-0" onClick={(e) => e.stopPropagation()}>
            {onUpdateFormat && !isProcessing && !isComplete && (
              <div className="flex items-center gap-1 shrink-0">
                ...
                <select
                  ...
                  className="px-1 py-0.5 text-[10px] font-bold border rounded bg-neutral-50 dark:bg-[#202124] border-neutral-200 dark:border-[#3c4043] text-neutral-700 dark:text-[#e8eaed] focus:outline-none focus:border-blue-500 cursor-pointer font-sans"
            ...
            {onReformatItem && isComplete && !isProcessing && (
              <div className="flex items-center gap-1 shrink-0">
                <span className="text-[10px] font-bold text-emerald-600 dark:text-[#81c995] select-none">To:</span>
                <select
                  value={item.customTargetFormat || ''}
                  onChange={(e) => {
                    const val = e.target.value;
                  ...
                  className="px-1 py-0.5 text-[10px] font-bold border rounded bg-emerald-50 dark:bg-[#1a2c20]/50 border-emerald-200 dark:border-[#2d523c] text-emerald-800 dark:text-[#81c995] focus:outline-none focus:border-emerald-500 cursor-pointer font-sans"
                >
            ...
            {!isProcessing && (onEditItem || onInspectDetails) && (
              <button
                ...
                className="px-1.5 py-0.5 transition-all rounded text-[10px] font-bold text-neutral-600 dark:text-[#e8eaed] bg-neutral-100 dark:bg-[#3c4043] hover:bg-blue-50 dark:hover:bg-blue-950/50 hover:text-blue-600 dark:hover:text-[#8ab4f8] border border-neutral-200/80 dark:border-[#5f6368] flex items-center gap-1 shrink-0 cursor-pointer"
                title="Edit per-file options"
                aria-label="Edit image options"
              >
                <Sliders className="w-3 h-3 text-blue-600 dark:text-[#8ab4f8]" />
                <span>Edit</span>
              </button>
            )}
          </div>
          ...
          <div className="flex flex-wrap items-center gap-1.5 ml-auto justify-end">
            {isComplete && item.savedToFolder && (
              <span 
                className="px-1.5 py-0.5 text-[9px] font-black rounded bg-emerald-50 dark:bg-[#1a2c20] text-emerald-700 dark:text-[#81c995] border border-emerald-200 dark:border-[#2d523c] flex items-center gap-1 cursor-default shrink-0"
            ...
            {isComplete && !item.savedToFolder && onCompare && (
              <button
                ...
                className="p-1 transition-all rounded hover:bg-blue-50 dark:hover:bg-[#1e293b] hover:text-blue-600 dark:hover:text-[#8ab4f8] shrink-0 cursor-pointer"
            ...
            {isComplete && !item.savedToFolder && (
              <button
                ...
                className="p-1 transition-all rounded hover:bg-neutral-100 dark:hover:bg-[#3c4043] hover:text-neutral-800 dark:text-[#9aa0a6] dark:hover:text-[#e8eaed] shrink-0 cursor-pointer"
                title="Download"
            ...
            {isError && onRetry && (
              <button
                ...
                className="p-1 transition-all rounded hover:bg-neutral-100 dark:hover:bg-[#3c4043] hover:text-blue-600 dark:hover:text-[#8ab4f8] shrink-0 cursor-pointer"
        ...
        {!isProcessing && (onRotate || onInspectDetails || onSelectRegions) && (
          <div className="flex flex-wrap items-center justify-between gap-1.5 pt-1 sm:pt-1.5 border-t border-dashed border-neutral-100 dark:border-[#3c4043]/40 w-full">
            <span className="text-[9px] font-extrabold text-neutral-400 dark:text-[#9aa0a6] tracking-wider uppercase select-none hidden min-[360px]:inline-block">Quick Tools</span>
            <div className="flex flex-wrap items-center gap-1.5 justify-end">
              {onRotate && (
                <div className="flex items-center gap-0.5">
                  <button
                    onClick={(e) => { e.stopPropagation(); onRotate(item.id, -90); }}
                    className="p-1 transition-all rounded text-neutral-400 hover:bg-neutral-100 dark:hover:bg-[#3c4043] hover:text-indigo-600 dark:hover:text-[#a8b1ff] shrink-0 cursor-pointer"
              ...
              {onInspectDetails && (
                <button
                  ...
                  className="p-1 transition-all rounded text-neutral-400 hover:bg-indigo-50 dark:hover:bg-[#21243a] hover:text-indigo-600 dark:hover:text-[#a8b1ff] shrink-0 cursor-pointer"
              ...
              <button
                ...
                className="p-1 transition-all rounded text-neutral-400 hover:bg-neutral-100 dark:hover:bg-[#3c4043] hover:text-neutral-800 dark:hover:text-[#e8eaed] shrink-0 cursor-pointer"
