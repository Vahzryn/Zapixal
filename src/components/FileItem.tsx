...
function FileItemComponent({ 
  ...
  return (
    <div 
      ...
      className={cn(
        "flex flex-col min-[520px]:flex-row min-[520px]:items-center justify-between p-3 sm:p-3.5 transition-all bg-white dark:bg-[#303134] border-2 rounded-2xl group shadow-sm select-none gap-2.5 min-w-0 w-full",
    ...
    >
      ...
      <div className="flex items-center flex-1 min-w-0 gap-2.5">
        ...
        {onToggleSelect && (
          <div className="shrink-0 flex items-center justify-center pl-1 pr-0.5">
            <div 
              className={cn(
        ...
        <div className="relative shrink-0 w-12 h-12 sm:w-13 sm:h-13 overflow-hidden rounded-xl bg-neutral-100 dark:bg-[#202124] border border-neutral-200/60 dark:border-[#3c4043] shadow-sm flex items-center justify-center">
      ...
      {/* Action Tools & Status Block */}
      <div className="flex items-center justify-end gap-1 shrink-0 pt-2 min-[520px]:pt-0 border-t min-[520px]:border-t-0 border-neutral-100 dark:border-[#3c4043]/60 min-[520px]:border-l-2 min-[520px]:pl-2 min-[520px]:ml-1">
        {/* Rotate Controls */}
        ...
        {isProcessing && (
          <Loader2 className="w-5 h-5 animate-spin text-blue-500 dark:text-[#8ab4f8] my-1 shrink-0" />
        )}
        ...
        {isComplete && (
          <>
            ...
            <CheckCircle2 className="w-5 h-5 text-emerald-500 dark:text-[#81c995] shrink-0 ml-0.5" />
          </>
        )}
        ...
        {isError && (
          <AlertCircle className="w-5 h-5 text-red-500 dark:text-[#f28b82] shrink-0 ml-0.5" />
        )}
        ...
        {(item.status === 'pending' || item.status === 'error' || isComplete) && (
          <button
            ...
            className={cn(
              "p-1.5 transition-all rounded-xl shrink-0 ml-0.5 cursor-pointer",
