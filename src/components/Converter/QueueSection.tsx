...
export const QueueSection = React.memo<QueueSectionProps>(function QueueSection({
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-8 duration-500">
      ...
      {hasDirectoryPicker && (
        <div 
          className="p-3.5 bg-neutral-50 dark:bg-neutral-900 border border-neutral-200/80 dark:border-neutral-800 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-left"
          ...
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <span className="text-xs font-semibold text-neutral-900 dark:text-white">Direct Folder Save</span>
                {directoryHandle && (
              ...
              <p className="text-[11px] text-neutral-500 dark:text-neutral-400 truncate max-w-md">
                {directoryHandle 
          ...
          <div className="flex items-center gap-2 shrink-0 self-end sm:self-center">
            {directoryHandle ? (
              ...
              <button
                ...
                disabled={isProcessing}
                className="px-3 py-1.5 text-xs font-medium text-neutral-700 dark:text-neutral-200 bg-white dark:bg-neutral-800 hover:bg-neutral-100 dark:hover:bg-neutral-700 border border-neutral-200 dark:border-neutral-700 rounded-lg transition-colors cursor-pointer disabled:opacity-50"
      ...
      <div className="flex flex-col bg-white dark:bg-neutral-900 border rounded-2xl border-neutral-200 dark:border-neutral-800 overflow-hidden shadow-xs">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 px-5 py-3.5 border-b border-neutral-100 dark:border-neutral-800 bg-neutral-50/60 dark:bg-neutral-900/60">
          <div className="flex flex-wrap items-center gap-2">
            <span className="font-bold text-neutral-900 dark:text-white text-sm">Files ({files.length})</span>
            {successCount > 0 && (
            ...
            {pendingCount > 0 && (
              <span className="px-2 py-0.5 text-[11px] font-medium rounded-md bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400">
                {pendingCount} pending
              </span>
            )}
            {isProcessing && etaText && (
              <span className="px-2 py-0.5 text-[11px] font-medium rounded-md bg-blue-50 dark:bg-blue-950/50 text-blue-700 dark:text-blue-300">
                {etaText}
              </span>
            )}
          </div>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
            <button
              ...
              disabled={isProcessing}
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg bg-neutral-100 hover:bg-neutral-200/70 dark:bg-neutral-800 dark:hover:bg-neutral-700 text-neutral-700 dark:text-neutral-200 transition-colors disabled:opacity-50 cursor-pointer"
            ...
            <button 
              onClick={onClearAll}
              disabled={isProcessing}
              className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium text-neutral-500 hover:text-red-600 dark:text-neutral-400 dark:hover:text-red-400 transition-colors disabled:opacity-50 cursor-pointer"
